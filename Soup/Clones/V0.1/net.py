...
def make_model(num_words, seq_length):
	...
	model = Sequential()
	model.add(Embedding(num_words, 50, input_length=seq_length))
...
def make_sequences():
	...
	:return:
	'''
#	in_filename = "../data/" + FILE_PREFIX + '.txt'
#	text = get_text_from_file(in_filename)
	text = ''
	for fil in songs:
		if not ('skit' in fil):
...
#	out_filename = FILE_PREFIX + '_sequences.txt'
	out_filename = 'Slimshady_LP' + '_sequences.txt'
